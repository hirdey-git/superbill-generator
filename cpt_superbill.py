...
if menu == "MDM CPT Generator":
    ...
    if submitted:
        ...
        try:
            ...
            level = determine_mdm_level_with_ai(mdm_text, mapping_rows)
            minutes = extract_or_estimate_time(mdm_text)
            matched, by_time = match_cpt(mapping_rows, level, minutes)
            improved = suggest_higher_billing_mdm_and_cpt(mdm_text, mapping_rows)
            ...
            exec_write(
                ...
                (
                    ...
                    by_time,
                    f"Selected via {'time' if by_time else 'MDM'}",
                    improved,
                    None
                )
            )
